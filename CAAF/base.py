...
class Base(object):
    """
    Base class implements the state, structure, and history of the cellular
    automaton lattice. The dynamics of the lattice are not included.
    """
    ...
    def __init__(
        ...
        self.driving_period = (
            driving_period if driving_period is not None else refractory_period * 2
        )
    ...
    def _initialise_connections(self):
        """
        Returns a random array specifying whether a cell is connected to each of
        its four neighbours.
        """
        connections = np.random.choice(
            [0, 1],
            size=(self.row_size, self.col_size, 4),
            p=[1 - self.prob_con, self.prob_con],
        ).astype(np.bool_)
    ...
    def _initialise_defects(self):
        ...
        return np.random.choice(
            [0, (1 - self.prob_not_fire)],
            size=(self.row_size, self.col_size),
